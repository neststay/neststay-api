...
- JWT `sub` claim continues to carry the ulid (now the `slug` column) — unchanged in shape. A JWT payload is only base64-encoded, not encrypted, so putting the internal bigint id there would leak it to anyone holding the token; the bigint id must never appear in a JWT. `JwtStrategy.validate()` now resolves the full user via a `findBySlug` lookup so the internal bigint id is available server-side. **BREAKING**: `CurrentUser()` now yields the resolved `bigint` id (instead of the raw `sub` string) for use in downstream bigint-keyed queries.
...
- `user-login`: login response returns `slug` instead of `id`; JWT `sub` continues to carry the ulid `slug` (never the bigint id).
...
- `jwt-auth-guard`: `JwtStrategy.validate()` resolves the JWT `sub` (ulid slug) to the user via `findBySlug`; `@CurrentUser()` now returns the resolved internal bigint id instead of the raw `sub` string.
...
- **Auth**: `src/auth/strategies/jwt.strategy.ts` — `validate()` now looks up the user by `slug` (via `UserService`/`UserRepository`) instead of passing the `sub` claim straight through, so it can return the internal bigint id; `src/auth/decorators/current-user.decorator.ts` — now extracts the bigint id from the resolved user.
