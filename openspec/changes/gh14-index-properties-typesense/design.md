...
**6. Typesense document also includes `imageUrls`, an ordered array of the property's image URLs.**
A follow-up change (search query API against this collection) needs to render result cards without a second, per-item Postgres round-trip for images — the same "no N+1 in lists" principle `docs/architecture/index.md` already applies to the DB-backed listing endpoint. `PropertyService.getBySlug` (used by the processor) already returns `images` ordered by `order` ascending via the existing repository include, so no repository change is needed — the processor maps `property.images.map(i => i.url)` onto the document. The field is stored/indexed but not marked `facet: true` (a list of URLs is display data, not a filter dimension) and is excluded from `query_by` (URLs aren't meaningfully full-text searchable).

**7. Indexing failures are isolated and do not affect property creation or retry indefinitely.**
