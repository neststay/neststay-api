...
**Non-Goals:**
- Storing search results or click/conversion events against a `search_history` row (explicit future work — this change only reserves `slug` as the id that future conversion tracking would key off).
...
**4. `search_history.slug` (ulid) is the id returned to the client as `searchId`, generated the same way `Property.slug` is.**
Follows the existing convention (`ulid()` called in the repository's `create`, e.g. `property.repository.ts:73`) rather than inventing a new id scheme. This keeps the "public-facing rows get a slug" rule consistent and gives the client a single opaque token to hold onto for later click/conversion correlation — the same reason `Property` exposes `slug` instead of its BigInt `id`.
...
Query params (`locationId`, `placeTypeId`, `numberOfGuests`, `numberOfBedrooms`, `numberOfBathrooms`, `minNightlyRate`/`maxNightlyRate`) are all optional and additive — omitted params impose no filter. `facet_by` is fixed to `locationId,placeTypeId,numberOfGuests,numberOfBedrooms,numberOfBathrooms` (mirrors the collection's `facet: true` fields from gh14) so the response always includes facet counts for those dimensions regardless of which filters the caller applied, letting the client render "narrow further" UI.
