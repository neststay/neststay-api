...
- **THEN** each resulting `search_history` row has a distinct `slug`
...
- **THEN** the response includes a `searchId` field matching the `slug` of the corresponding `search_history` row
