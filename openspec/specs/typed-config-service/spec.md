...
Compile-time-checked access to application configuration via a strictly-typed `ConfigService`, covering all existing config namespaces (`app`, `cors`, `database`, `jwt`, `redis`, `queue`) derived from the `registerAs` factories in `src/config/`.
