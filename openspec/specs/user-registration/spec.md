...
- **THEN** a `user.register` job SHALL be added to the `user-events` BullMQ queue with `userId`, `email`, and `name` (no `password`)
...
The system SHALL return HTTP `201 Created` with a `ResponseApiDto` envelope where `data` contains only `{ id, email }`.

#### Scenario: Registration success response
- **WHEN** `POST /users/register` completes successfully
- **THEN** the response status SHALL be `201 Created`
- **AND** the response body SHALL match `{ success: true, message: <string>, data: { id: <string>, email: <string> } }`
- **AND** the `data` object SHALL NOT include `password`, `name`, `createdAt`, or `updatedAt`
